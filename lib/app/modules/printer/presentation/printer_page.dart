...
// Platform messages are asynchronous, so we initialize in an async method.
  Future<void> initBluetooth() async {
    try {
      ...
      if (mounted) {
        setState(() => tips = bluetoothPrintUserMessage(e));
      }
    } catch (_) {
      if (mounted) {
        setState(() => tips = 'Não foi possível procurar impressoras.');
      }
    }
                              ...
                              : () async {
                                  if (_device != null &&
                                    ...
                                    try {
                                      ...
                                      if (!mounted) return;
                                      setState(
                                          () => tips = bluetoothPrintUserMessage(e));
                                    } catch (_) {
                                      if (!mounted) return;
                                      setState(() => tips = 'Falha ao conectar.');
                                    }
                                  } else {
                                    setState(() {
                      ...
                      onPressed: _connected
                          ? () async {
                              ...
                              try {
                                ...
                                if (!mounted) return;
                                setState(
                                    () => tips = 'Falha no teste de impressão.');
                              }
                            }
                          : null,
