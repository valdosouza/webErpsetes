...
class OrderStockAdjustRegisterBloc
    extends Bloc<OrderStockAdjustRegisterEvent, OrderStockAdjustRegisterState> {
  ...
  OrderStockAdjustMainModel orderMain = OrderStockAdjustMainModel.empty();
  ...
  int indexOrder = 0;
  OrderStockAdjustItemModel elementItemDelete =
      OrderStockAdjustItemModel.empty();

  OrderStockAdjustRegisterBloc({
  ...
  Future<String> _validateOrder() async {
    if (orderMain.orderStockAdjust.nameEntity.isEmpty) {
      return "Campo Entidade é Obrigatório";
    }
    if (orderMain.orderStockAdjust.nameStockList.isEmpty) {
